...
function LoginForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await login({ // login now expects { username, password }
        username: formState.formData.identifier.trim(),
  ...
  // Lỗi sẽ được xử lý và hiển thị qua AuthContext error (như "Invalid username or password!" )
    } catch (err) {
      ...
      <Row className="justify-content-md-center">
        <Col xs={12} md={6}>
          <Card>
            ...
            <Card.Body>
              {/* Hiển thị lỗi từ AuthContext ("Invalid username or password!") */}
              {error && (
